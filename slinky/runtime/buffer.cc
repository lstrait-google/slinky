...
bool calculate_flat_bounds(std::size_t rank, const dim* dims, index_t& flat_min,
                           index_t& flat_max) {
  flat_min = 0;
  flat_max = 0;
  bool overflow = false;
  ...
  for (std::size_t i = 0; i < rank; ++i) {
    ...
    index_t term_min;
    index_t term_max;
    overflow = overflow || sub_with_overflow(
        extent, static_cast<index_t>(1), extent_minus_1);
    overflow = overflow || mul_with_overflow(
        extent_minus_1, std::min<index_t>(0, stride),
                          term_min);
    overflow = overflow || add_with_overflow(flat_min, term_min, flat_min);
    overflow = overflow || mul_with_overflow(
        extent_minus_1, std::max<index_t>(0, stride),
                          term_max);
...
std::size_t alloc_size(std::size_t rank, std::size_t elem_size,
                       const dim* dims) {
  for (std::size_t i = 0; i < rank; ++i) {
...
void pad_impl(raw_buffer& src, raw_buffer& dst, raw_buffer& pad) {
  for (int d = static_cast<int>(std::min(src.rank, dst.rank)) - 1; d >= 0; --d) {
    const slinky::dim& src_d = src.dims[d];
...
bool validate_buffer(const raw_buffer& buf) {
  ...
  if (add_with_overflow(flat_max, elem_size_u, max_offset)) return false;
...
SLINKY_INLINE bool can_fuse(span<const raw_buffer*, BufsSize> bufs, std::size_t d) {
  ...
  if (base_outer.stride() != 0 && base_inner.fold_factor() > 0) {
